...
class _DashatarCountdownState extends State<DashatarCountdown> {
  ...
  Widget build(BuildContext context) {
    ...
    // final puzzleProcess = context.watch<PuzzleState>().puzzleProcess;

    return AudioControlListener(
      audioPlayer: _audioPlayer,
      child: BlocListener<DashatarPuzzleBloc, DashatarPuzzleState>(
        listener: (context, state) {
          if (!state.isCountdownRunning && !state.isReqReverse) {
            return;
          }

          // Play the shuffle sound when the countdown from 3 to 1 begins.
          if (state.secondsToBegin == 3 && !state.isReqReverse) {
          ...
          if (state.secondsToBegin >= 1 &&
              state.secondsToBegin <= 3 &&
              !state.isReqReverse) {
            context.read<PuzzleBloc>().add(const PuzzleReset());
          }

          // Reverse the puzzle on every countdown tick.
          // create:
          if (state.isReqReverse) {
            context.read<PuzzleBloc>().add(PuzzleReverse(puzzle.tiles));
            unawaited(_audioPlayer.replay());
          }
        },
